...
if st.session_state["DETAILS"] == None:
    ...
    with st.spinner("Checking your access privileges and Loading content..."):
        logcheck = requests.get(
            f"https://school.deta.dev/data/check/admin?username={st.session_state['DETAILS']['username']}&password={st.session_state['DETAILS']['password']}"
        )
        ...
        if logcheck.json()["admin"] == True:
            ...
            with c1:
                ...
                users = requests.get(f"https://school.deta.dev/data/user")
                num = 1
                for user in users.json():
                    with st.container():
                        ...
                        if user["admin"] == True:
                            ...
                            if p:
                                pu = requests.patch(
                                    f"https://school.deta.dev/data/update/user?value=admin&state=False&id={user['id']}"
                                )
                        ...
                        if user["admin"] == False:
                            ...
                            if m:
                                pu = requests.patch(
                                    f"https://school.deta.dev/data/update/user?value=admin&state=True&id={user['id']}"
                                )
                        ...
                        if user["banned"] == True:
                            ...
                            if u:
                                pu = requests.patch(
                                    f"https://school.deta.dev/data/update/user?value=banned&state=False&id={user['id']}"
                                )
                        ...
                        if user["banned"] == False:
                            ...
                            if b:
                                pu = requests.patch(
                                    f"https://school.deta.dev/data/update/user?value=banned&state=True&id={user['id']}"
                                )
                                st.success("User banned")
                    num += 1
            with c2:
                st.markdown("# Manage reports")
                reports = requests.get(f"https://school.deta.dev/data/wall")
                num = 1
                for report in reports.json():
                    if report["reported"] == True:
                        with st.container():
                            ...
                            if r:
                                pu = requests.delete(
                                    f"https://school.deta.dev/data/delete/wall?id={report['key']}"
                                )
